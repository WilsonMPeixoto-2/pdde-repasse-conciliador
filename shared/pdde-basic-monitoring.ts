...
export function derivePddeBasicPortfolio(schools: readonly PddeBasicSchoolLike[]) {
  ...
  return {
    ...
    firstPaymentInformedCents: rows.reduce((total, row) => total + row.first.paymentInformedCents, 0),
    ...
    secondPaymentInformedCents: rows.reduce((total, row) => total + row.second.paymentInformedCents, 0),
    ...
    secondRegularPaidCount: rows.filter((row) => row.second.track === 'PDDE Básico' && row.second.state === 'PAID_INFORMED').length,
    secondInfancyPaidCount: rows.filter((row) => row.second.track === 'Primeira Infância' && row.second.state === 'PAID_INFORMED').length,
