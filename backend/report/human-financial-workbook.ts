...
function dataRows(sheet: ExcelJS.Worksheet, start: number): void {
...
function moneyColumns(sheet: ExcelJS.Worksheet, columns: number[]): void {
...
function uniquePrograms(school: HumanFinancialSchoolView): string[] {
  return [...new Set(school.programs.map((program) => program.name))].sort((a, b) => a.localeCompare(b, 'pt-BR'));
}

function latestSchoolPosition(school: HumanFinancialSchoolView): { cents: number | null; date: string | null } {
  ...
  const known = latest.map((position) => position.totalReportedBalanceCents);
  if (known.some((value) => value === null)) return { cents: null, date: latestDate };
  return {
    cents: known.reduce((sum, value) => sum + (value ?? 0), 0),
    date: latestDate,
  };
}

function buildOverview(workbook: ExcelJS.Workbook, view: HumanFinancialPortfolioView): void {
  ...
  const installments = view.schools.flatMap((school) => school.programs.flatMap((program) => program.installments));
  const accounts = view.schools.flatMap((school) => school.accounts);
  const movements = accounts.flatMap((account) => account.movements);
  const programmed = installments.reduce((sum, item) => sum + item.programmedCents, 0);
  const informed = installments.reduce((sum, item) => sum + item.paymentInformedCents, 0);
  const latestBalances = view.schools.map(latestSchoolPosition);
  const totalBalanceKnown = latestBalances.every((item) => item.cents !== null)
    ? latestBalances.reduce((sum, item) => sum + (item.cents ?? 0), 0)
    : null;

  sheet.addRow([]);
  const metricsHeader = sheet.addRow([
    'Unidades', 'Contas acompanhadas', 'Movimentações em 2026',
    'Previsto', 'Pagamento informado', 'Saldo mais recente',
  ]);
  ...
  const notes = [
    ['PDDEInfo', 'Repasses informados, contas vinculadas, posição de saldo/aplicações e situação da prestação de contas.'],
    ['SIGEF', 'Movimentações da conta e créditos compatíveis localizados no extrato.'],
    ['Data de referência', 'O saldo apresentado corresponde à posição publicada para a data indicada; não representa automaticamente o saldo do dia da consulta.'],
  ...
  sheet.columns = [
    { width: 21 }, { width: 24 }, { width: 24 }, { width: 21 }, { width: 22 }, { width: 22 },
  ];
}
...
function buildUnits(workbook: ExcelJS.Workbook, view: HumanFinancialPortfolioView): void {
  ...
  subtitle(sheet, 'Visão resumida da carteira. Selecione a unidade desejada nas demais abas para detalhar repasses, contas e movimentações.', 9);
  header(sheet.addRow(['SME', 'Unidade escolar', 'INEP', 'UEx', 'CNPJ', 'Programas', 'Contas', 'Saldo mais recente', 'Posição do saldo']));
  for (const school of view.schools) {
    ...
    sheet.addRow([
      ...
      safeText(school.school.uex), safeText(school.school.cnpj), uniquePrograms(school).join(' · '),
      school.accounts.length, reais(position.cents), brDate(position.date),
    ]);
  }
...
function buildAccounting(workbook: ExcelJS.Workbook, view: HumanFinancialPortfolioView): void {
  ...
  subtitle(sheet, 'Situação informada na fonte pública na data da coleta. Acompanhe cada programa separadamente.', 7);
  header(sheet.addRow([
