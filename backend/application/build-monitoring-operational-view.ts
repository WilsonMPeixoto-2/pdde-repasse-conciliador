...
export interface OperationalMovement {
  id: string;
  school: { inep: string; sme: string; name: string; cnpj: string };
  programCode: string;
  programLabel: string;
  account: BankAccount;
  movementDate: string;
...
export interface OperationalRepasse {
  ...
  /** Candidatos preservados para investigação; não constituem créditos atribuídos. */
  bankCreditCandidates?: Array<{ id: string; date: string; amountCents: number; document: string | null }>;
}
...
const CLASSIFICATIONS: SigefMovementClass[] = [
  'REPASSE_FNDE', 'APLICACAO_FINANCEIRA', 'RESGATE_APLICACAO', 'PAGAMENTO_TRANSFERENCIA',
  'PAGAMENTO_CARTAO', 'RENDIMENTO_FINANCEIRO', 'ENTRADA_TERCEIRO', 'TARIFA_BANCARIA',
  'ESTORNO_REVERSAO', 'MOVIMENTO_NAO_CLASSIFICADO',
];
...
export function refineOperationalMovementClass(rawClassification: string, operation: 'credit' | 'debit', rawHistory: string): SigefMovementClass {
  if (CLASSIFICATIONS.includes(rawClassification as SigefMovementClass) && rawClassification !== 'MOVIMENTO_NAO_CLASSIFICADO') return rawClassification as SigefMovementClass;
  const history = canonicalText(rawHistory);
  if (operation === 'debit' && history === 'TV POR ASSINATURA') return 'PAGAMENTO_TRANSFERENCIA';
  if (operation === 'credit' && history.startsWith('TED TRANSFERENCIA ELETR DISPON')) return 'ENTRADA_TERCEIRO';
  if (operation === 'credit' && history === 'TRANSFERIDO DA POUPANCA') return 'RESGATE_APLICACAO';
...
function repasseBreakdown(repasse: z.infer<typeof repasseSchema>): OperationalRepasse['breakdown'] {
  const values = [repasse.programadoCusteioCents, repasse.programadoCapitalCents, repasse.ajusteCusteioCents, repasse.ajusteCapitalCents, repasse.pagoCusteioCents, repasse.pagoCapitalCents];
...
function baseRepasse(school: z.infer<typeof schoolSchema>, repasse: z.infer<typeof repasseSchema>, account: BankAccount | null) {
  return {
    school: { inep: school.inep, sme: school.sme, name: school.name, cnpj: school.cnpj },
    programCode: repasse.programCode, action: repasse.action, installment: repasse.installment,
    amountProgrammedCents: repasse.programadoCents, amountPaidInformedCents: repasse.pagoInformadoCents,
    ...(repasseBreakdown(repasse) ? { breakdown: repasseBreakdown(repasse) } : {}),
    orderDate: repasse.dataOrdem, account,
  };
}
...
function markSharedCreditCandidates(repasses: OperationalRepasse[]): void {
  const claimants = new Map<string, Set<OperationalRepasse>>();
  for (const repasse of repasses) for (const candidate of repasse.bankCreditCandidates ?? []) {
    const key = JSON.stringify([repasse.school.inep, repasse.programCode, accountKey(repasse.account), candidate.id]);
    const bucket = claimants.get(key) ?? new Set<OperationalRepasse>(); bucket.add(repasse); claimants.set(key, bucket);
  }
  for (const bucket of claimants.values()) if (bucket.size > 1) for (const repasse of bucket) {
    repasse.bankCreditStatus = 'CREDITO_AMBIGUO'; repasse.bankCreditDate = null; repasse.bankCreditAmountCents = null; repasse.bankDocument = null; repasse.daysAfterOrder = null;
  }
}
...
export function buildMonitoringOperationalView(rawInput: unknown) {
  ...
  for (const school of input.schools) for (const accountResult of school.accounts) for (const movement of accountResult.movements) {
  ...
  for (const movement of movements) { classificationCounts[movement.classification] += 1; classificationAmountsCents[movement.classification] += movement.amountCents; }
  ...
  for (const repasse of repasses) { repasseStatusCounts[repasse.bankCreditStatus] = (repasseStatusCounts[repasse.bankCreditStatus] ?? 0) + 1; repasseStatusAmountsCents[repasse.bankCreditStatus] = (repasseStatusAmountsCents[repasse.bankCreditStatus] ?? 0) + repasse.amountPaidInformedCents; }
  const alerts: MonitoringAlert[] = [];
  for (const repasse of repasses) if (['PAGO_SEM_CONTA_ATUAL','PAGO_CREDITO_NAO_LOCALIZADO','CREDITO_AMBIGUO','CONSULTA_INCONCLUSIVA'].includes(repasse.bankCreditStatus)) alerts.push({ kind: 'REPASSE', schoolInep: repasse.school.inep, schoolName: repasse.school.name, programCode: repasse.programCode, date: repasse.orderDate, amountCents: repasse.amountPaidInformedCents, message: repasse.bankCreditStatus, reference: null });
  for (const movement of movements) {
    if (movement.classification === 'TARIFA_BANCARIA') alerts.push({ kind: 'TARIFA', schoolInep: movement.school.inep, schoolName: movement.school.name, programCode: movement.programCode, date: movement.movementDate, amountCents: movement.amountCents, message: 'Tarifa bancária detectada; verificar cabimento e eventual estorno.', reference: movement.document || null });
    else if (movement.classification === 'ENTRADA_TERCEIRO') alerts.push({ kind: 'ENTRADA_EXTERNA', schoolInep: movement.school.inep, schoolName: movement.school.name, programCode: movement.programCode, date: movement.movementDate, amountCents: movement.amountCents, message: 'Entrada externa à ordem bancária FNDE; verificar origem antes de qualquer conclusão.', reference: movement.counterparty.document });
    else if (movement.classification === 'MOVIMENTO_NAO_CLASSIFICADO') alerts.push({ kind: 'MOVIMENTO_REVISAR', schoolInep: movement.school.inep, schoolName: movement.school.name, programCode: movement.programCode, date: movement.movementDate, amountCents: movement.amountCents, message: `Histórico sem semântica suficiente para classificação automática: ${movement.history}`, reference: movement.document || null });
  }
  const confirmedDelays = repasses.filter((repasse) => repasse.bankCreditStatus === 'CREDITO_CONFIRMADO' && repasse.daysAfterOrder !== null).map((repasse) => repasse.daysAfterOrder as number);
  return { version: 1, generatedAt: new Date().toISOString(), sourceGeneratedAt: input.generatedAt, fiscalYear: input.fiscalYear, sourceStatus: input.status, sources: input.sources, sourceObservations: input.sourceObservations, coverage: input.coverage, summary: { schools: input.schools.length, repasses: repasses.length, movementsInFiscalYear: movements.length, repasseStatusCounts, repasseStatusAmountsCents, classificationCounts, classificationAmountsCents, confirmedCreditDelayDays: confirmedDelays.length > 0 ? { min: Math.min(...confirmedDelays), max: Math.max(...confirmedDelays) } : null, explicitYieldMovementsFound: classificationCounts.RENDIMENTO_FINANCEIRO }, repasses, movements: movements.sort((left, right) => right.movementDate.localeCompare(left.movementDate) || left.school.inep.localeCompare(right.school.inep) || left.id.localeCompare(right.id)), alerts };
}
