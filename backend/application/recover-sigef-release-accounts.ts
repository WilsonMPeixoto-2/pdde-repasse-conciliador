...
import { collectSigefPublicReleases, type SigefPublicReleaseCollection } from '../adapters/sigef-public-releases';
import { collectSigefPublicAccount, type SigefAccountResult, type SigefMovementClass } from '../adapters/sigef-public-statement';
...
import { buildMonitoringSourceObservations, buildReleaseSourceObservation } from '../core/source-observation';
...
type CollectSigefReleases = (input: { cnpj: string; programCode: string; fiscalYear: number; targetCnpjs?: string[]; signal?: AbortSignal }) => Promise<SigefPublicReleaseCollection>;
...
export interface SigefReleaseAccountRecovery {
  schoolInep: string; schoolCnpj: string; programCode: string; action: string; installment: string | null;
  amountCents: number; status: 'RECOVERED' | 'CONFIRMED' | 'ACCOUNT_MISMATCH' | 'NOT_FOUND' | 'AMBIGUOUS' | 'ERROR';
  account: BankAccount | null; paymentDate: string | null; orderBank: string | null; sourceUrl: string | null; error: string | null;
  /** Candidatos compatíveis preservados quando a associação não é única. */ candidates?: SigefRelease[];
}
export interface SigefReleaseQueryObservation {
  schoolInep: string; schoolCnpj: string; programCode: string; status: 'COMPLETE' | 'ERROR'; queriedAt: string;
  sourceUrl: string | null; route: 'modern' | 'legacy' | null; rawSha256: string | null; releases: SigefRelease[]; error: string | null;
}
export type MonitoringWithSigefReleaseAccounts = Omit<RunMonitoringResult, 'raw'> & { raw: RawMonitoring & { accountRecoveries: SigefReleaseAccountRecovery[]; releaseQueries: SigefReleaseQueryObservation[] } };
export interface RecoverSigefReleaseAccountsOptions { base: RunMonitoringResult; workspacePath: string; fiscalYear: 2026; signal?: AbortSignal; collectSigefReleases?: CollectSigefReleases; collectSigefAccount?: CollectSigefAccount; now?: () => string }

function emptyTotals(): Record<SigefMovementClass, number> { return Object.fromEntries(CLASSIFICATIONS.map((classification) => [classification, 0])) as Record<SigefMovementClass, number>; }
function safeSegment(value: string): string { return value.replace(/[^0-9A-Za-z._-]/g, '_'); }
async function mapConcurrent<T,R>(items: readonly T[], concurrency: number, worker: (item: T) => Promise<R>): Promise<R[]> {
  ...
  async function run(): Promise<void> { while (true) { const index = cursor; cursor += 1; if (index >= items.length) return; results[index] = await worker(items[index]); } }
...
function actionCode(programCode: string, action: string): string | null {
  ...
  if (programCode === '02') { if (text.includes('PRIMEIRA INFANCIA')) return 'PDDE_PRIMEIRA_INFANCIA'; if (text.includes('PDDE BASICO') || text.includes('MANUTENCAO ESCOLAR')) return 'PDDE_BASICO'; }
  if (programCode === '0B') { if (text.includes('EDUCACAO CONECTADA')) return 'EDUCACAO_CONECTADA'; if (text.includes('ESCOLA E COMUNIDADE')) return 'ESCOLA_E_COMUNIDADE'; if (text.includes('ESCOLA DAS ADOLESCENCIAS')) return 'ESCOLA_DAS_ADOLESCENCIAS'; if (text.includes('CANTINHO DA LEITURA')) return 'CANTINHO_DA_LEITURA'; }
  if (programCode === '0A' && (text.includes('PDDE SRM') || text.includes('SALA DE RECURSOS'))) return 'PDDE_SRM';
  if (programCode === 'Z9' && text.includes('EDUCACAO INTEGRAL')) return 'PDDE_EDUCACAO_INTEGRAL'; return null;
}
...
function failedAccountResult(input: { schoolInep:string; programCode:string; programLabel:string; account:BankAccount; error:string }): RawAccount { return { inep:input.schoolInep, programCode:input.programCode, programLabel:input.programLabel, account:input.account, saldoPddeInfoCents:null, occurrence:null, status:'ERROR', error:input.error, pagesFetched:0, declaredTotal:null, uniqueMovements:0, movementsInYear:0, coverageThrough:null, totals:emptyTotals(), movements:[] } as RawAccount; }
function recomputeRaw(raw: RawMonitoring & { accountRecoveries: SigefReleaseAccountRecovery[] }, releaseFailures:number): void {
  const accounts=raw.schools.flatMap((school)=>school.accounts); const repasses=raw.schools.flatMap((school)=>school.repasses); const totals=emptyTotals(); let historical=0,movementsInYear=0,balances=0,complete=0,partial=0,failed=0;
...
export async function recoverSigefReleaseAccounts(options: RecoverSigefReleaseAccountsOptions): Promise<MonitoringWithSigefReleaseAccounts> {
  const releaseCollector=options.collectSigefReleases??collectSigefPublicReleases; const statementCollector=options.collectSigefAccount??collectSigefPublicAccount; const workspacePath=resolve(options.workspacePath); const raw=structuredClone(options.base.raw) as MonitoringWithSigefReleaseAccounts['raw']; raw.accountRecoveries=[]; raw.releaseQueries=[]; const now=options.now??(()=>new Date().toISOString()); const escalationKeys=needsReleaseEscalation(options);
  const groups:Array<{school:RawSchool;programCode:string;repasses:RawRepasse[]}>=[];
  for(const school of raw.schools){const pendingByProgram=new Map<string,RawRepasse[]>();for(const repasse of school.repasses){if(repasse.pagoInformadoCents<=0)continue;const key=repasseKey({schoolInep:school.inep,programCode:repasse.programCode,action:repasse.action,installment:repasse.installment,amountCents:repasse.pagoInformadoCents,orderDate:repasse.dataOrdem});if(!escalationKeys.has(key))continue;const list=pendingByProgram.get(repasse.programCode)??[];list.push(repasse);pendingByProgram.set(repasse.programCode,list);}for(const [programCode,repasses] of pendingByProgram)groups.push({school,programCode,repasses});}
