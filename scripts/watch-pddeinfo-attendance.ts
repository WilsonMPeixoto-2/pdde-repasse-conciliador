...
import { fetchPddeInfoBulkAttendanceReport } from '../backend/adapters/pddeinfo-public-reports';
import {
...
async function collectAttendance(
  ...
  if (missing.length > 0) {
    throw new Error(
      `Exportação agregada do PDDEInfo não cobriu ${missing.length}/163 UEs da carteira: `
